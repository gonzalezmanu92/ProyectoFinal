...
from .models import Monto_Final, Monto_Inicial, Pago_Impuesto, Size, Category, Topping, Price_List, Item_List, Cart_List, Extra, Order
...
def guardar_dato_cierrecaja(request):
	if request.method == "POST":
		...
		cierre = Monto_Final(montofinal=valor)
		cierre.save()
		return redirect('pagina_exitosa_cierrecaja')
	return render(request, "orders/cierredecaja.html")

def guardar_pago_impuesto(request):
	if request.method == "POST":
		nameimpuesto = request.POST.get("nameimpuesto")
		montopagar = request.POST.get("montopagar")
		impuesto = Pago_Impuesto(nameimpuesto=nameimpuesto,montopagar=montopagar)
		impuesto.save()
		return redirect('pagina_exitosa_pagoimpuesto')
	return render(request, "orders/pagoimpuestos.html")
